...
void OmnidirectionalCamera::RingStitch(cv::Mat left, cv::Mat right, cv::Mat dst, int v_diff, int blend_width)
{
	...
	for (int i = 0; i < height - v_diff; i++)
	{
		...
		for (int j = ((dst_width / 2) - blend_width); j < (dst_width / 2); j++)
		{
			...
			p_dst[j][0] = (int)((double)p_left[j + blend_width][0] * (1 - alpha) + (double)p_right[Bindex][0] * alpha);
			p_dst[j][1] = (int)((double)p_left[j + blend_width][1] * (1 - alpha) + (double)p_right[Bindex][1] * alpha);
			p_dst[j][2] = (int)((double)p_left[j + blend_width][2] * (1 - alpha) + (double)p_right[Bindex][2] * alpha);

			p_dst[j + (dst_width / 2)][0] = (int)((double)p_right[j + blend_width][0] * (1 - alpha) + (double)p_left[j - ((dst_width / 2) - blend_width)][0] * alpha);
			p_dst[j + (dst_width / 2)][1] = (int)((double)p_right[j + blend_width][1] * (1 - alpha) + (double)p_left[j - ((dst_width / 2) - blend_width)][1] * alpha);
			p_dst[j + (dst_width / 2)][2] = (int)((double)p_right[j + blend_width][2] * (1 - alpha) + (double)p_left[j - ((dst_width / 2) - blend_width)][2] * alpha);
		}
	}
}